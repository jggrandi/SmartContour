...
/**
*/
NRRDFileManager::NRRDFileManager(void)
{
}
...
/**
*/
	//bool NRRDFileManager::LoadFromFile(Nrrd* NRRDimage, int imgNumber)
bool NRRDFileManager::LoadFromFile(boost::shared_ptr<CGageAdaptor> NRRDimage, int imgNumber)
{
...
/*	if (m_bitsAllocated != 16)
	...
	//m_width = NRRDimage->axis[0].size;//GetWidth();
	//m_height = NRRDimage->axis[1].size;//GetHeight();
	m_width = NRRDimage->GetWidth();
	m_height = NRRDimage->GetHeight();
	if (m_width <= 0)
	{
		MarkError();
	...
	//short* data;
	//memcpy(NRRDimage->data, data, m_width*m_height*43*sizeof(short));
	for (int i=0; i<m_height; ++i){
		for (int j=0; j<m_width; ++j){
		//	short* porradocaralho= NRRDimage->data[(imgNumber*m_width*m_height)+(i*m_width+j)];
			fileData.push_back(/*(short*)NRRDimage->data[(imgNumber*m_width*m_height)+(i*m_width+j)]);//*/NRRDimage->GetValue(j, i, imgNumber));
		}	
	}
	m_sliceLocation = NRRDimage->GetDepth()-imgNumber;
...
//	fileDataEnd = (short *)&(*fileData.end());//fileSize;

	m_imageData.reserve(m_width*m_height);//fileSize);
	...
	for (int k=0; k<m_width*m_height; k++){
		m_imageData.push_back(fileData[k]);
	}
...
/**
*/
bool NRRDFileManager::LoadDataFromFile(const std::string& directoryName)
{
	/*char fileName[1024];
	ifstream fileStream;
	istreambuf_iterator<char> fileStreamBegin,
